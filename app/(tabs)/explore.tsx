...
const LOGIC_TYPES = new Set(['series', 'intruder', 'logicMatch', 'pattern', 'thinkNumber']);
...
export default function LogicTab() {
  const items = useMemo(
    () =>
      levels
        ...
        .flatMap((level) =>
          level.exercises
            .filter((exercise) => LOGIC_TYPES.has(exercise.type))
            .map((exercise) => ({
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>Logica</Text>
